	...
	<div id="reviewList_board">
		...
		<div class="reviewList_head_sub_text">
			<p class="reviewList_sub_text">
				...
				<br> <span class="reviewList_span">오묘가묘와 함께 하세요♡</span> 주인의 세심한
				선택이 아이에게 특별한 행복으로 각인됩니다.
			</p>
		</div>
		<div class="reviewList_button">
			...
			<form action="reviewListBoard.do" method="post">
				<table border="1">
					<tr>
						<td><select name="searchCondition" class="select_search">
								<option value="TITLE">제목</option>
								<option value="CONTENT">내용</option>
						</select> <input type="text" name="searchKeyword" class="text_input" /> <input
							type="submit" value="검색" class="submit_button" /></td>
					</tr>
				</table>
			</form>
		</div>
		...
		<div class="reviewList_page00">
			<a href="reviewListBoard.do" class="reviewList_page01">&lt;&lt;</a> <a
				href="reviewListBoard.do?page=${navi.startPageGroup-1}"
				class="reviewList_page01">&lt;</a>
			<c:forEach var="counter" begin="${navi.startPageGroup}"
				end="${navi.endPageGroup}">
				<c:if test="${page.cnt == counter}">
					<a href="reviewListBoard.do?page=${counter}"
						class="reviewList_page" style="background-color: orange"
						onlcick="changeColor(${counter})">&nbsp;&nbsp;${counter}</a>
				</c:if>
				<c:if test="${page.cnt != counter}">
					<a href="reviewListBoard.do?page=${counter}"
						class="reviewList_page" onlcick="changeColor(${counter})">&nbsp;&nbsp;${counter}</a>
				</c:if>
			</c:forEach>
			<a href="reviewListBoard.do?page=${navi.endPageGroup+1}"
				class="reviewList_page01">&gt;</a> <a
				href="reviewListBoard.do?page=${navi.totalRecordsCount}"
				class="reviewList_page01">&gt;&gt;</a>
		</div>
	</div>
